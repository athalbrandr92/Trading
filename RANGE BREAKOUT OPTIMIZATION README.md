...
Open range minute: the minute at which to form the range, looking back over {Lookback minutes} number of minutes. I.E. 8AM Start Hour with 30 Open range minute with 30 lookback minutes calculates its daily range as the highest and lowest values between 8:00 to 8:30 AM.

Lookback minutes: How many minutes to look back over - these should typically be stepped in increments of 5, 10, or 15 minute intervals, though some traders do swear by a one-minute opening range on some assets. Note - much higher timeframes (h1+) start to lose meaning just a bit as far as a daily range is concerned. Thanks to my most recent upgrade, it technically can work on the daily chart or higher, but I haven't the foggiest idea how that might work out for you. Best of luck to you if you go that route.
...
Blackout Start and End months and days: Numerical months and days to begin and end the blackout on. Defaults to 12/20 - 1/5. Feel free to push the start up as late as Christmas Eve if you know you'll be closing your positions manually on Dec 23 and drop the end to as early as January 2. The important thing is to be out of the market from Christmas to New Year's. *Update* The optimizer now only allows you to set December and January as the start and end, with dates between the 10th and 24th in December and between the 2nd and 10th in January.
...
***Note - For the TP methods below, to save time and prevent yourself from confusing the genetic optimizer with meaningless extra passes, uncheck whichever of these do not apply to your current TP method.  While the latest update does Stop() passes with potentially repeating data, that still takes some time and CPU power.***

***Note #2 - if you're confident you have the *ENTIRE* rest of the strategy optimized and want to run the genetic optimizer to test which TP method and values perform best for those specific settings, uncheck everything else and check all TP method boxes - if you want to test both TP methods and Trailing Stop methods after fully optimizing for time and filters, do the same but also check all trailing SL boxes. Thanks to the latest update, the optimizer will automatically kill any pass with potential repeat parameters - if that TP or SL method is not selected and that TP or SL method's input is not set to default, it will print an error message and move on to the next pass.***

Primary TP Mode: how to set the TP.The options are Fixed (just as it sounds), Pivot Points (daily R1/R2/R3/S1/S2/S3 levels, now accepting 1/2 levels and extended out to R8/S8), SessionExtrema (maximum high and low of x number of prior days - currently bugged and needs fixed along with needing the name updated), and Trailing Only, which is just an open position with no take profit. Since I removed the prior HalfandHalf mode, you'll now need to run two separate instances if running a half and half strategy - just cut your risk in half and load one instance as {your TP type} and the other instance as Trailing Only.

R:R Ratio (Long) and (Short): these tell the bot how far out to place the take profit in either direction if Primary TP Mode is set to fixed. Flat multiple of the final SL distance (including the buffer).
...
***Note - For the trailing SL methods below, to save time and prevent yourself from confusing the genetic optimizer with meaningless extra passes, uncheck whichever of these do not apply to your current trailing SL method. While the latest update does Stop() passes with potentially repeating data, that still takes some time and CPU power.***

***Note #2 - if you're confident you have the *ENTIRE* rest of the strategy optimized and want to run the genetic optimizer to test which trailing SL method and values perform best for those specific settings, uncheck everything else and check all trailing SL method boxes - if you want to test both TP methods and Trailing Stop methods after fully optimizing for time and filters, do the same but also check all TP method boxes. Thanks to the latest update, the optimizer will automatically kill any pass with potential repeat parameters - if that TP or SL method is not selected and that TP or SL method's input is not set to default, it will print an error message and move on to the next pass.***
...
EMA Trail Period: Lookback for the EMA trail. I highly recommend doing something that seems kinda retarded at first glance - 49 to 147 by 7 gives decent results. If you're a stickler for 10s, 50-150 by 10 is also decent. Uncheck if not specifically using the EMA trail method.

Extrema Lookback: how many bars to look back for extrema if using that stop loss method. I'll be quite honest - I have no idea exactly what this should be set to. I haven't really gotten good results consistently enough to know. I'd recommend if you're going to try it to start with a broad pass of something like 5 or 10 to 100 by either 5 or 10 and narrow it after getting a consistent range. Uncheck if not specifically using the Extrema method.

PSAR Min AF and Max AF: Settings for the PSAR acceleration factor (how quickly it tightens in to the price as the trend continues). I usually set these to 0.01-0.1 by 0.01 and 0.1-1 by 0.1 if I'm using the PSAR trail. Uncheck if not specifically using the PSAR method.

Chandelier Mult: Multiplier for the Chandelier ATR. I optimize this as either 0.5-4 by 0.5 by 0.25 - I'm currently running some optimizations with the range tightened to 1.75-3.5 by 0.25 because most of my best results have come from that range. Uncheck if not specifically using the Chandelier method.
