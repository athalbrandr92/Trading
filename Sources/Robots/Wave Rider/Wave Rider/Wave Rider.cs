...
namespace cAlgo.Robots
{
    ...
    public class WaveRider : Robot
    {
        ...
        protected override void OnStart()
        {
            ...
            // --- UPDATED INDICATOR INITIALIZATION ---
            // Order must precisely match the DynamicSnRBoxes Parameter definitions
            _snrIndicator = Indicators.GetIndicator<DynamicSnRBoxes>(
                IndShowMultiday, 
                IndShowPreviousDay, 
                IndShowAsianSession, 
                IndShowLondonSession, 
                IndShowNySession, 
                IndShowPsychCenturies,   // Added
                IndShowPsychHalves,      // Added
                IndShowPsychQuartiles,   // Added
                IndShowOrderBlocks, 
                IndShowDoubleTopsBottoms, 
                IndShowConsolidation, 
                IndShowRejections,       // Added IndShowRejections specifically
                MacroAtrMult, 
                MicroAtrMult, 
                IndPsychLevelStep, 
                IndUtcOffset, 
                IndMultidayTimeFrame, 
                IndMultidayLookback, 
                IndAsianStartHour, 
                IndAsianEndHour, 
                IndLondonStartHour, 
                ...
                IndNyStartHour, 
                IndNyEndHour, 
                IndMinFormationCandles, 
                IndMaxLookbackCandles, 
        ...
        protected override void OnBar()
        {
            ...
            else if (rejMajorSell || brkMajorSell) ExecuteWave(ZoneTier.Major, TradeType.Sell);

            if (rejMidBuy || brkMidBuy) ExecuteWave(ZoneTier.Mid, TradeType.Buy);
            else if (rejMidSell || brkMidSell) ExecuteWave(ZoneTier.Mid, TradeType.Sell);

            if (rejMinorBuy || brkMinorBuy) ExecuteWave(ZoneTier.Minor, TradeType.Buy);
            else if (rejMinorSell || brkMinorSell) ExecuteWave(ZoneTier.Minor, TradeType.Sell);
        }
        ...
        private void HandleTrailingStops()
        {
            foreach (var pos in Positions.Where(p => p.SymbolName == SymbolName))
            {
                ...
                if (newSl.HasValue)
                {
                    if (pos.TradeType == TradeType.Buy && newSl > pos.StopLoss && newSl < currentPrice) ModifyPosition(pos, newSl, pos.TakeProfit);
                    else if (pos.TradeType == TradeType.Sell && newSl < pos.StopLoss && newSl > currentPrice) ModifyPosition(pos, newSl, pos.TakeProfit);
                }
            }
        }
